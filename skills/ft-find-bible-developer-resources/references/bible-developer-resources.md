...
| [**Bible Glide MCP**](https://www.bibleglide.com/mcp) | Rust / Remote MCP | Hosted Streamable HTTP endpoint: `https://api.bibleglide.com/mcp`. Reads connected Bible Glide account context including Daily Scripture, Lectionary Daily Scripture, small-group lessons, reader highlights/comments, recent community activity, and upcoming church events. Also exposes Bible Glide chat for explanation, synthesis, prayer wording, interpretation, and reflection. Uses OAuth so users approve access before account data is connected. |
